#!/bin/bash
set -e



RAY_REPO="https://raw.githubusercontent.com/NotRayy01/hosting/refs/heads/main/pterodactyl_manager"

fn_exists() { declare -F "$1" >/dev/null; }
if ! fn_exists lib_loaded; then
  source <(curl -sSL "$RAY_REPO/lib/lib.sh")
  ! fn_exists lib_loaded && echo "* ERROR: Could not load lib script" && exit 1
fi

# Override default lib functions with Ray UI
output() { step "$1"; }
success() { ok "$1"; }
error() { err "$1"; }
warning() { warn "$1"; }

RM_PANEL="${RM_PANEL:-true}"
RM_WINGS="${RM_WINGS:-true}"

# ---------- Uninstallation functions ---------- #

rm_panel_files() {
  step "Removing panel files..."
  rm -rf /var/www/pterodactyl /usr/local/bin/composer
  [ "$OS" != "centos" ] && [ -L /etc/nginx/sites-enabled/pterodactyl.conf ] && unlink /etc/nginx/sites-enabled/pterodactyl.conf
  [ "$OS" != "centos" ] && [ -f /etc/nginx/sites-available/pterodactyl.conf ] && rm -f /etc/nginx/sites-available/pterodactyl.conf
  [ "$OS" != "centos" ] && [ ! -L /etc/nginx/sites-enabled/default ] && [ -f /etc/nginx/sites-available/default ] && ln -s /etc/nginx/sites-available/default /etc/nginx/sites-enabled/default
  [ "$OS" == "centos" ] && [ -f /etc/nginx/conf.d/pterodactyl.conf ] && rm -f /etc/nginx/conf.d/pterodactyl.conf
  systemctl restart nginx || true
  ok "Removed panel files."
}

rm_docker_containers() {
  step "Removing docker containers and images..."
  docker system prune -a -f >/dev/null 2>&1 || true
  ok "Removed docker containers and images."
}

rm_wings_files() {
  step "Removing wings files..."
  systemctl disable --now wings >/dev/null 2>&1 || true
  [ -f /etc/systemd/system/wings.service ] && rm -rf /etc/systemd/system/wings.service
  [ -d /etc/pterodactyl ] && rm -rf /etc/pterodactyl
  [ -f /usr/local/bin/wings ] && rm -rf /usr/local/bin/wings
  [ -d /var/lib/pterodactyl ] && rm -rf /var/lib/pterodactyl
  ok "Removed wings files."
}

rm_services() {
  step "Removing services..."
  systemctl disable --now pteroq >/dev/null 2>&1 || true
  rm -rf /etc/systemd/system/pteroq.service
  case "$OS" in
  debian | ubuntu) systemctl disable --now redis-server >/dev/null 2>&1 || true ;;
  centos)
    systemctl disable --now redis >/dev/null 2>&1 || true
    systemctl disable --now php-fpm >/dev/null 2>&1 || true
    rm -rf /etc/php-fpm.d/www-pterodactyl.conf
    ;;
  esac
  ok "Removed services."
}

rm_cron() {
  step "Removing cron jobs..."
  crontab -l | grep -vF "* * * * * php /var/www/pterodactyl/artisan schedule:run >> /dev/null 2>&1" | crontab - || true
  ok "Removed cron jobs."
}

rm_database() {
  step "Removing database..."
  if ! command -v mariadb >/dev/null 2>&1 && ! command -v mysql >/dev/null 2>&1; then
      warn "MariaDB/MySQL not found. Skipping database removal."
      return
  fi

  valid_db=$(mariadb -u root -e "SELECT schema_name FROM information_schema.schemata;" 2>/dev/null | grep -v -E -- 'schema_name|information_schema|performance_schema|mysql')
  if [[ -z "$valid_db" ]]; then
    warn "No valid databases found."
    return
  fi

  warn "Be careful! This database will be deleted!"
  if [[ "$valid_db" == *"panel"* ]]; then
    echo -e -n " ${CYAN}➤${NC} Database 'panel' detected. Is this the Pterodactyl database? (y/N): "
    read -r is_panel
    if [[ "$is_panel" =~ [Yy] ]]; then
      DATABASE=panel
    else
      print_list "$valid_db"
    fi
  else
    print_list "$valid_db"
  fi

  while [ -z "$DATABASE" ] || [[ "$valid_db" != *"$DATABASE"* ]]; do
    echo -e -n " ${CYAN}➤${NC} Choose the panel database (leave blank to skip): "
    read -r database_input
    if [[ -n "$database_input" ]]; then
      if [[ "$valid_db" == *"$database_input"* ]]; then
        DATABASE="$database_input"
      else
        warn "Invalid database name. Try again."
      fi
    else
      break
    fi
  done

  if [[ -n "$DATABASE" ]]; then
    mariadb -u root -e "DROP DATABASE $DATABASE;" 2>/dev/null || warn "Failed to drop database $DATABASE."
    ok "Database $DATABASE dropped."
  else
    info "No database selected, skipping removal."
  fi

  step "Removing database user..."
  valid_users=$(mariadb -u root -e "SELECT user FROM mysql.user;" 2>/dev/null | grep -v -E -- 'user|root')
  if [[ -z "$valid_users" ]]; then
    warn "No valid database users found."
    return
  fi

  warn "Be careful! This user will be deleted!"
  if [[ "$valid_users" == *"pterodactyl"* ]]; then
    echo -e -n " ${CYAN}➤${NC} User 'pterodactyl' detected. Is this the Pterodactyl user? (y/N): "
    read -r is_user
    if [[ "$is_user" =~ [Yy] ]]; then
      DB_USER=pterodactyl
    else
      print_list "$valid_users"
    fi
  else
    print_list "$valid_users"
  fi

  while [ -z "$DB_USER" ] || [[ "$valid_users" != *"$DB_USER"* ]]; do
    echo -e -n " ${CYAN}➤${NC} Choose the panel user (leave blank to skip): "
    read -r user_input
    if [[ -n "$user_input" ]]; then
      if [[ "$valid_users" == *"$user_input"* ]]; then
        DB_USER=$user_input
      else
        warn "Invalid username. Try again."
      fi
    else
      break
    fi
  done

  if [[ -n "$DB_USER" ]]; then
    mariadb -u root -e "DROP USER '$DB_USER'@'127.0.0.1';" 2>/dev/null || warn "Failed to drop user $DB_USER."
    ok "User $DB_USER dropped."
  else
    info "No user selected, skipping removal."
  fi

  mariadb -u root -e "FLUSH PRIVILEGES;" 2>/dev/null || true
}

perform_uninstall() {
  [ "$RM_PANEL" == true ] && rm_panel_files
  [ "$RM_PANEL" == true ] && rm_cron
  [ "$RM_PANEL" == true ] && rm_database
  [ "$RM_PANEL" == true ] && rm_services
  [ "$RM_WINGS" == true ] && rm_docker_containers
  [ "$RM_WINGS" == true ] && rm_wings_files
  return 0
}

perform_uninstall